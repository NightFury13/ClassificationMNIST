function [Y, R, E] = Isomap1(D,lab, n_fcn, n_size, options); 
...
for di = 1:length(dims)
     if (dims(di)<=N)
         ...
         r2 = 1-corrcoef(reshape(real(tangent_d(Y.coords{di}, Y.coords{di})),N^2,1),D).^2; 
         R(di) = r2(2,1); 
         if (verbose == 1)
...
%%%%%%%%%%%%%%%%%% Graphics %%%%%%%%%%%%%%%%%%
	...
	if (displ==1)
     ...
     if ~isempty(twod)
		 ...
		 for i=1:10
         plot(Y.coords{twod}(1,find(lab==i)), Y.coords{twod}(2,find(lab==i)), Markerstyle(i),'MarkerSize',5,'MarkerEdgeColor',Colors(i));
		 end
         if (overlay == 1)
